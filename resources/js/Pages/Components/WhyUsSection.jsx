import { IoRocketOutline } from "react-icons/io5";

export default function WhyUsSection() {
    return (
        <section id="whychooseus">
            <h2 className="text-4xl font-bold text-white"># Why Choose Us?</h2>
            <div className="grid xl:grid-cols-2 my-4">
                <div className="flex items-start gap-4 rounded-lg bg-white p-6 shadow-[0px_14px_34px_0px_rgba(0,0,0,0.08)] ring-1 ring-white/[0.05] transition duration-300 hover:text-black/70 hover:ring-black/20 focus:outline-none focus-visible:ring-[#FF2D20] lg:pb-10 dark:bg-opacity-80 dark:bg-zinc-900 dark:ring-zinc-800 dark:hover:text-white/70 dark:hover:ring-zinc-700 dark:focus-visible:ring-[#FF2D20]">
                    <div className="flex size-12 shrink-0 p-1 items-center justify-center rounded-full bg-[#FF2D20]/10 sm:size-16">
                        {/* <svg
                            className="size-5 sm:size-6"
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            viewBox="0 0 24 24"
                        >
                            <g fill="#FF2D20">
                                <path d="M24 8.25a.5.5 0 0 0-.5-.5H.5a.5.5 0 0 0-.5.5v12a2.5 2.5 0 0 0 2.5 2.5h19a2.5 2.5 0 0 0 2.5-2.5v-12Zm-7.765 5.868a1.221 1.221 0 0 1 0 2.264l-6.626 2.776A1.153 1.153 0 0 1 8 18.123v-5.746a1.151 1.151 0 0 1 1.609-1.035l6.626 2.776ZM19.564 1.677a.25.25 0 0 0-.177-.427H15.6a.106.106 0 0 0-.072.03l-4.54 4.543a.25.25 0 0 0 .177.427h3.783c.027 0 .054-.01.073-.03l4.543-4.543ZM22.071 1.318a.047.047 0 0 0-.045.013l-4.492 4.492a.249.249 0 0 0 .038.385.25.25 0 0 0 .14.042h5.784a.5.5 0 0 0 .5-.5v-2a2.5 2.5 0 0 0-1.925-2.432ZM13.014 1.677a.25.25 0 0 0-.178-.427H9.101a.106.106 0 0 0-.073.03l-4.54 4.543a.25.25 0 0 0 .177.427H8.4a.106.106 0 0 0 .073-.03l4.54-4.543ZM6.513 1.677a.25.25 0 0 0-.177-.427H2.5A2.5 2.5 0 0 0 0 3.75v2a.5.5 0 0 0 .5.5h1.4a.106.106 0 0 0 .073-.03l4.54-4.543Z" />
                            </g>
                        </svg> */}
                        <svg
                            fill="#000000"
                            height="193px"
                            width="193px"
                            version="1.1"
                            id="Capa_1"
                            xmlns="http://www.w3.org/2000/svg"
                            xmlns:xlink="http://www.w3.org/1999/xlink"
                            viewBox="-110.73 -110.73 664.37 664.37"
                            xml:space="preserve"
                            stroke="#000000"
                            stroke-width="3.986154"
                        >
                            <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                            <g
                                id="SVGRepo_tracerCarrier"
                                stroke-linecap="round"
                                stroke-linejoin="round"
                                stroke="#fbff00"
                                stroke-width="30.117608"
                            >
                                {" "}
                                <g>
                                    {" "}
                                    <path d="M442.485,250.247c-1.053-3.01-3.892-5.025-7.08-5.025h-71.493V19.741c0-4.142-3.357-7.5-7.5-7.5H86.495 c-4.142,0-7.5,3.358-7.5,7.5v225.481H7.5c-3.188,0-6.027,2.015-7.08,5.025c-1.052,3.009-0.087,6.355,2.406,8.341L216.78,429.03 c1.367,1.089,3.02,1.634,4.673,1.634s3.306-0.544,4.673-1.634l213.952-170.442C442.572,256.602,443.537,253.257,442.485,250.247z M221.453,413.575L28.951,260.222h57.544c4.142,0,7.5-3.358,7.5-7.5V27.241h254.917v225.481c0,4.142,3.357,7.5,7.5,7.5h57.542 L221.453,413.575z"></path>{" "}
                                    <path d="M155.968,171.393h27.329c4.142,0,7.5-3.358,7.5-7.5s-3.358-7.5-7.5-7.5h-19.829v-47.157c0-4.142-3.358-7.5-7.5-7.5 s-7.5,3.358-7.5,7.5v54.657C148.468,168.035,151.826,171.393,155.968,171.393z"></path>{" "}
                                    <path d="M289.104,171.073c3.167-0.956,5.334-3.873,5.334-7.181v-54.657c0-4.142-3.357-7.5-7.5-7.5s-7.5,3.358-7.5,7.5v29.791 l-4.397-6.626c-1.39-2.094-3.735-3.353-6.249-3.353s-4.859,1.259-6.249,3.353l-4.397,6.626v-29.791c0-4.142-3.357-7.5-7.5-7.5 s-7.5,3.358-7.5,7.5v54.657c0,3.308,2.167,6.225,5.334,7.181c3.17,0.956,6.588-0.278,8.415-3.034l11.897-17.929l11.897,17.929 c1.415,2.133,3.783,3.354,6.25,3.354C287.659,171.393,288.388,171.289,289.104,171.073z"></path>{" "}
                                    <path d="M213.904,101.736c-11.67,0-21.165,9.494-21.165,21.164v27.329c0,11.67,9.494,21.165,21.165,21.165 s21.165-9.495,21.165-21.165V122.9C235.068,111.23,225.574,101.736,213.904,101.736z M220.068,150.228 c0,3.399-2.766,6.165-6.165,6.165s-6.165-2.766-6.165-6.165V122.9c0-3.399,2.766-6.164,6.165-6.164s6.165,2.765,6.165,6.164 V150.228z"></path>{" "}
                                    <path d="M321.41,219.883c4.143,0,7.5-3.358,7.5-7.5s-3.357-7.5-7.5-7.5h-27.329c-4.143,0-7.5,3.358-7.5,7.5v54.657 c0,4.142,3.357,7.5,7.5,7.5h27.329c4.143,0,7.5-3.358,7.5-7.5s-3.357-7.5-7.5-7.5h-19.829v-12.329h7.916c4.143,0,7.5-3.358,7.5-7.5 s-3.357-7.5-7.5-7.5h-7.916v-12.329H321.41z"></path>{" "}
                                    <path d="M223.491,204.883c-4.143,0-7.5,3.358-7.5,7.5v54.657c0,4.142,3.357,7.5,7.5,7.5s7.5-3.358,7.5-7.5v-54.657 C230.991,208.241,227.634,204.883,223.491,204.883z"></path>{" "}
                                    <path d="M272.183,219.883c4.143,0,7.5-3.358,7.5-7.5s-3.357-7.5-7.5-7.5c-19.204,0-34.828,15.624-34.828,34.829 s15.624,34.829,34.828,34.829c4.143,0,7.5-3.358,7.5-7.5s-3.357-7.5-7.5-7.5c-10.934,0-19.828-8.895-19.828-19.829 S261.25,219.883,272.183,219.883z"></path>{" "}
                                    <path d="M135.16,204.883h-13.664c-4.142,0-7.5,3.358-7.5,7.5v54.657c0,4.142,3.358,7.5,7.5,7.5s7.5-3.358,7.5-7.5v-19.829h6.164 c11.67,0,21.165-9.494,21.165-21.164S146.83,204.883,135.16,204.883z M135.16,232.212h-6.164v-12.329h6.164 c3.399,0,6.165,2.766,6.165,6.165S138.559,232.212,135.16,232.212z"></path>{" "}
                                    <path d="M187.488,246.766c9.6-2.004,16.834-10.533,16.834-20.719c0-11.67-9.494-21.165-21.165-21.165h-13.664 c-4.142,0-7.5,3.358-7.5,7.5v27.269c0,0.039,0,0.078,0,0.117v27.272c0,4.142,3.358,7.5,7.5,7.5s7.5-3.358,7.5-7.5v-9.556 l15.331,14.929c2.967,2.89,7.717,2.828,10.605-0.141c2.89-2.967,2.827-7.716-0.141-10.605L187.488,246.766z M176.993,232.212 v-12.329h6.164c3.399,0,6.165,2.766,6.165,6.165s-2.766,6.164-6.165,6.164H176.993z"></path>{" "}
                                </g>{" "}
                            </g>
                            <g id="SVGRepo_iconCarrier">
                                {" "}
                                <g>
                                    {" "}
                                    <path d="M442.485,250.247c-1.053-3.01-3.892-5.025-7.08-5.025h-71.493V19.741c0-4.142-3.357-7.5-7.5-7.5H86.495 c-4.142,0-7.5,3.358-7.5,7.5v225.481H7.5c-3.188,0-6.027,2.015-7.08,5.025c-1.052,3.009-0.087,6.355,2.406,8.341L216.78,429.03 c1.367,1.089,3.02,1.634,4.673,1.634s3.306-0.544,4.673-1.634l213.952-170.442C442.572,256.602,443.537,253.257,442.485,250.247z M221.453,413.575L28.951,260.222h57.544c4.142,0,7.5-3.358,7.5-7.5V27.241h254.917v225.481c0,4.142,3.357,7.5,7.5,7.5h57.542 L221.453,413.575z"></path>{" "}
                                    <path d="M155.968,171.393h27.329c4.142,0,7.5-3.358,7.5-7.5s-3.358-7.5-7.5-7.5h-19.829v-47.157c0-4.142-3.358-7.5-7.5-7.5 s-7.5,3.358-7.5,7.5v54.657C148.468,168.035,151.826,171.393,155.968,171.393z"></path>{" "}
                                    <path d="M289.104,171.073c3.167-0.956,5.334-3.873,5.334-7.181v-54.657c0-4.142-3.357-7.5-7.5-7.5s-7.5,3.358-7.5,7.5v29.791 l-4.397-6.626c-1.39-2.094-3.735-3.353-6.249-3.353s-4.859,1.259-6.249,3.353l-4.397,6.626v-29.791c0-4.142-3.357-7.5-7.5-7.5 s-7.5,3.358-7.5,7.5v54.657c0,3.308,2.167,6.225,5.334,7.181c3.17,0.956,6.588-0.278,8.415-3.034l11.897-17.929l11.897,17.929 c1.415,2.133,3.783,3.354,6.25,3.354C287.659,171.393,288.388,171.289,289.104,171.073z"></path>{" "}
                                    <path d="M213.904,101.736c-11.67,0-21.165,9.494-21.165,21.164v27.329c0,11.67,9.494,21.165,21.165,21.165 s21.165-9.495,21.165-21.165V122.9C235.068,111.23,225.574,101.736,213.904,101.736z M220.068,150.228 c0,3.399-2.766,6.165-6.165,6.165s-6.165-2.766-6.165-6.165V122.9c0-3.399,2.766-6.164,6.165-6.164s6.165,2.765,6.165,6.164 V150.228z"></path>{" "}
                                    <path d="M321.41,219.883c4.143,0,7.5-3.358,7.5-7.5s-3.357-7.5-7.5-7.5h-27.329c-4.143,0-7.5,3.358-7.5,7.5v54.657 c0,4.142,3.357,7.5,7.5,7.5h27.329c4.143,0,7.5-3.358,7.5-7.5s-3.357-7.5-7.5-7.5h-19.829v-12.329h7.916c4.143,0,7.5-3.358,7.5-7.5 s-3.357-7.5-7.5-7.5h-7.916v-12.329H321.41z"></path>{" "}
                                    <path d="M223.491,204.883c-4.143,0-7.5,3.358-7.5,7.5v54.657c0,4.142,3.357,7.5,7.5,7.5s7.5-3.358,7.5-7.5v-54.657 C230.991,208.241,227.634,204.883,223.491,204.883z"></path>{" "}
                                    <path d="M272.183,219.883c4.143,0,7.5-3.358,7.5-7.5s-3.357-7.5-7.5-7.5c-19.204,0-34.828,15.624-34.828,34.829 s15.624,34.829,34.828,34.829c4.143,0,7.5-3.358,7.5-7.5s-3.357-7.5-7.5-7.5c-10.934,0-19.828-8.895-19.828-19.829 S261.25,219.883,272.183,219.883z"></path>{" "}
                                    <path d="M135.16,204.883h-13.664c-4.142,0-7.5,3.358-7.5,7.5v54.657c0,4.142,3.358,7.5,7.5,7.5s7.5-3.358,7.5-7.5v-19.829h6.164 c11.67,0,21.165-9.494,21.165-21.164S146.83,204.883,135.16,204.883z M135.16,232.212h-6.164v-12.329h6.164 c3.399,0,6.165,2.766,6.165,6.165S138.559,232.212,135.16,232.212z"></path>{" "}
                                    <path d="M187.488,246.766c9.6-2.004,16.834-10.533,16.834-20.719c0-11.67-9.494-21.165-21.165-21.165h-13.664 c-4.142,0-7.5,3.358-7.5,7.5v27.269c0,0.039,0,0.078,0,0.117v27.272c0,4.142,3.358,7.5,7.5,7.5s7.5-3.358,7.5-7.5v-9.556 l15.331,14.929c2.967,2.89,7.717,2.828,10.605-0.141c2.89-2.967,2.827-7.716-0.141-10.605L187.488,246.766z M176.993,232.212 v-12.329h6.164c3.399,0,6.165,2.766,6.165,6.165s-2.766,6.164-6.165,6.164H176.993z"></path>{" "}
                                </g>{" "}
                            </g>
                        </svg>
                    </div>

                    <div className="pt-3 sm:pt-5">
                        <h2 className="text-xl font-semibold text-black dark:text-white">
                            Affordable Prices
                        </h2>

                        <p className="mt-4 text-sm/relaxed">
                            High-quality solutions that fit your budget. We
                            deliver premium services without breaking the bank,
                            ensuring exceptional value for your investment.
                        </p>
                    </div>
                </div>
                <div></div>
            </div>
            <div className="grid xl:grid-cols-2 my-4">
                <div></div>
                <div className="flex items-start gap-4 rounded-lg bg-white p-6 shadow-[0px_14px_34px_0px_rgba(0,0,0,0.08)] ring-1 ring-white/[0.05] transition duration-300 hover:text-black/70 hover:ring-black/20 focus:outline-none focus-visible:ring-[#FF2D20] lg:pb-10 dark:bg-opacity-80 dark:bg-zinc-900 dark:ring-zinc-800 dark:hover:text-white/70 dark:hover:ring-zinc-700 dark:focus-visible:ring-[#FF2D20]">
                    <div className="flex size-12 shrink-0 p-1 items-center justify-center rounded-full bg-[#FF2D20]/10 sm:size-16">
                        {/* <svg
                            className="size-5 sm:size-6"
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            viewBox="0 0 24 24"
                        >
                            <g fill="#FF2D20">
                                <path d="M24 8.25a.5.5 0 0 0-.5-.5H.5a.5.5 0 0 0-.5.5v12a2.5 2.5 0 0 0 2.5 2.5h19a2.5 2.5 0 0 0 2.5-2.5v-12Zm-7.765 5.868a1.221 1.221 0 0 1 0 2.264l-6.626 2.776A1.153 1.153 0 0 1 8 18.123v-5.746a1.151 1.151 0 0 1 1.609-1.035l6.626 2.776ZM19.564 1.677a.25.25 0 0 0-.177-.427H15.6a.106.106 0 0 0-.072.03l-4.54 4.543a.25.25 0 0 0 .177.427h3.783c.027 0 .054-.01.073-.03l4.543-4.543ZM22.071 1.318a.047.047 0 0 0-.045.013l-4.492 4.492a.249.249 0 0 0 .038.385.25.25 0 0 0 .14.042h5.784a.5.5 0 0 0 .5-.5v-2a2.5 2.5 0 0 0-1.925-2.432ZM13.014 1.677a.25.25 0 0 0-.178-.427H9.101a.106.106 0 0 0-.073.03l-4.54 4.543a.25.25 0 0 0 .177.427H8.4a.106.106 0 0 0 .073-.03l4.54-4.543ZM6.513 1.677a.25.25 0 0 0-.177-.427H2.5A2.5 2.5 0 0 0 0 3.75v2a.5.5 0 0 0 .5.5h1.4a.106.106 0 0 0 .073-.03l4.54-4.543Z" />
                            </g>
                        </svg> */}
                        <svg
                            viewBox="-2.4 -2.4 28.80 28.80"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                            stroke="#d0fa00"
                        >
                            <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                            <g
                                id="SVGRepo_tracerCarrier"
                                stroke-linecap="round"
                                stroke-linejoin="round"
                            ></g>
                            <g id="SVGRepo_iconCarrier">
                                {" "}
                                <path
                                    fill-rule="evenodd"
                                    clip-rule="evenodd"
                                    d="M12.261 1.03462C12.6971 1.15253 13 1.54819 13 1.99997V8.99997H19C19.3581 8.99997 19.6888 9.19141 19.8671 9.50191C20.0455 9.8124 20.0442 10.1945 19.8638 10.5038L12.8638 22.5038C12.6361 22.8941 12.1751 23.0832 11.739 22.9653C11.3029 22.8474 11 22.4517 11 22V15H5C4.64193 15 4.3112 14.8085 4.13286 14.498C3.95452 14.1875 3.9558 13.8054 4.13622 13.4961L11.1362 1.4961C11.3639 1.10586 11.8249 0.916719 12.261 1.03462ZM6.74104 13H12C12.5523 13 13 13.4477 13 14V18.301L17.259 11H12C11.4477 11 11 10.5523 11 9.99997V5.69889L6.74104 13Z"
                                    fill="#000000"
                                ></path>{" "}
                            </g>
                        </svg>
                    </div>

                    <div className="pt-3 sm:pt-5">
                        <h2 className="text-xl font-semibold text-black dark:text-white">
                            Lightning-Fast Turnaround
                        </h2>

                        <p className="mt-4 text-sm/relaxed">
                            Time is crucial, and we respect it. Our efficient
                            processes and expert team ensure your project is
                            completed quickly without compromising quality.
                        </p>
                    </div>
                </div>
            </div>
            <div className="grid xl:grid-cols-2 my-4">
                <div className="flex items-start gap-4 rounded-lg bg-white p-6 shadow-[0px_14px_34px_0px_rgba(0,0,0,0.08)] ring-1 ring-white/[0.05] transition duration-300 hover:text-black/70 hover:ring-black/20 focus:outline-none focus-visible:ring-[#FF2D20] lg:pb-10 dark:bg-opacity-80 dark:bg-zinc-900 dark:ring-zinc-800 dark:hover:text-white/70 dark:hover:ring-zinc-700 dark:focus-visible:ring-[#FF2D20]">
                    <div className="flex size-12 shrink-0 items-center p-1 justify-center rounded-full bg-[#FF2D20]/10 sm:size-16">
                        <svg
                            fill="#1100ff"
                            version="1.1"
                            id="Capa_1"
                            xmlns="http://www.w3.org/2000/svg"
                            xmlns:xlink="http://www.w3.org/1999/xlink"
                            viewBox="-51.41 -51.41 674.02 674.02"
                            xml:space="preserve"
                            stroke="#1100ff"
                            stroke-width="0.005712"
                        >
                            <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                            <g
                                id="SVGRepo_tracerCarrier"
                                stroke-linecap="round"
                                stroke-linejoin="round"
                                stroke="#fbfbfe"
                                stroke-width="10.281600000000001"
                            >
                                {" "}
                                <g>
                                    {" "}
                                    <path d="M353.601,496.4c0,7.507-6.093,13.6-13.601,13.6H231.2c-7.507,0-13.6-6.093-13.6-13.6c0-7.508,6.093-13.601,13.6-13.601H340 C347.508,482.8,353.601,488.893,353.601,496.4z M340,516.8H231.2c-8.949,0-15.878,8.644-12.899,18.034 c1.795,5.664,7.527,9.166,13.471,9.166h0.204c7.854,0,15.035,4.44,18.55,11.465l0.143,0.286 c4.74,9.465,14.416,15.449,25.004,15.449h19.856c10.588,0,20.264-5.984,24.997-15.449l0.143-0.286 c3.516-7.024,10.696-11.465,18.55-11.465h0.204c5.943,0,11.676-3.502,13.471-9.166C355.878,525.443,348.949,516.8,340,516.8z M285.601,81.6c7.507,0,13.6-6.093,13.6-13.6V13.6c0-7.507-6.093-13.6-13.6-13.6C278.093,0,272,6.093,272,13.6V68 C272,75.507,278.093,81.6,285.601,81.6z M141.352,133.382c2.652,2.659,6.134,3.985,9.615,3.985c3.482,0,6.963-1.326,9.615-3.985 c5.311-5.311,5.311-13.92,0-19.23l-38.467-38.468c-5.304-5.311-13.927-5.311-19.23,0c-5.311,5.311-5.311,13.919,0,19.23 L141.352,133.382z M108.8,258.4c0-7.507-6.093-13.6-13.6-13.6H40.8c-7.507,0-13.6,6.093-13.6,13.6c0,7.507,6.093,13.6,13.6,13.6 h54.4C102.708,272,108.8,265.907,108.8,258.4z M141.352,383.418l-38.467,38.468c-5.311,5.311-5.311,13.92,0,19.23 c2.652,2.659,6.133,3.984,9.615,3.984c3.481,0,6.963-1.325,9.615-3.984l38.467-38.468c5.311-5.311,5.311-13.919,0-19.23 C155.278,378.107,146.656,378.107,141.352,383.418z M429.849,383.418c-5.311-5.311-13.92-5.311-19.23,0s-5.311,13.92,0,19.23 l38.468,38.468c2.658,2.659,6.134,3.984,9.615,3.984s6.956-1.325,9.615-3.984c5.311-5.311,5.311-13.92,0-19.23L429.849,383.418z M530.4,244.8H476c-7.507,0-13.6,6.093-13.6,13.6c0,7.507,6.093,13.6,13.6,13.6h54.4c7.507,0,13.6-6.093,13.6-13.6 C544,250.893,537.907,244.8,530.4,244.8z M420.233,137.367c3.481,0,6.956-1.326,9.615-3.985l38.468-38.468 c5.311-5.311,5.311-13.919,0-19.23c-5.311-5.311-13.92-5.311-19.23,0l-38.468,38.468c-5.311,5.311-5.311,13.919,0,19.23 C413.271,136.041,416.752,137.367,420.233,137.367z M353.601,462.4c0,7.507-6.093,13.6-13.601,13.6H231.2 c-7.507,0-13.6-6.093-13.6-13.6c0-7.242,5.678-13.11,12.818-13.519C221.952,372.354,142.8,355.307,142.8,265.2 c0-78.866,63.934-142.8,142.8-142.8c78.866,0,142.8,63.934,142.8,142.8c0,90.106-79.152,107.154-87.618,183.682 C347.922,449.29,353.601,455.158,353.601,462.4z M254.259,160.548c-2.115-5.216-8.051-7.725-13.287-5.624 c-34.755,14.083-61.104,44.186-70.482,80.525c-1.408,5.46,1.877,11.016,7.331,12.424c0.85,0.224,1.707,0.326,2.55,0.326 c4.542,0,8.684-3.053,9.874-7.65c7.766-30.11,29.594-55.053,58.385-66.715C253.851,171.721,256.367,165.777,254.259,160.548z"></path>{" "}
                                </g>{" "}
                            </g>
                            <g id="SVGRepo_iconCarrier">
                                {" "}
                                <g>
                                    {" "}
                                    <path d="M353.601,496.4c0,7.507-6.093,13.6-13.601,13.6H231.2c-7.507,0-13.6-6.093-13.6-13.6c0-7.508,6.093-13.601,13.6-13.601H340 C347.508,482.8,353.601,488.893,353.601,496.4z M340,516.8H231.2c-8.949,0-15.878,8.644-12.899,18.034 c1.795,5.664,7.527,9.166,13.471,9.166h0.204c7.854,0,15.035,4.44,18.55,11.465l0.143,0.286 c4.74,9.465,14.416,15.449,25.004,15.449h19.856c10.588,0,20.264-5.984,24.997-15.449l0.143-0.286 c3.516-7.024,10.696-11.465,18.55-11.465h0.204c5.943,0,11.676-3.502,13.471-9.166C355.878,525.443,348.949,516.8,340,516.8z M285.601,81.6c7.507,0,13.6-6.093,13.6-13.6V13.6c0-7.507-6.093-13.6-13.6-13.6C278.093,0,272,6.093,272,13.6V68 C272,75.507,278.093,81.6,285.601,81.6z M141.352,133.382c2.652,2.659,6.134,3.985,9.615,3.985c3.482,0,6.963-1.326,9.615-3.985 c5.311-5.311,5.311-13.92,0-19.23l-38.467-38.468c-5.304-5.311-13.927-5.311-19.23,0c-5.311,5.311-5.311,13.919,0,19.23 L141.352,133.382z M108.8,258.4c0-7.507-6.093-13.6-13.6-13.6H40.8c-7.507,0-13.6,6.093-13.6,13.6c0,7.507,6.093,13.6,13.6,13.6 h54.4C102.708,272,108.8,265.907,108.8,258.4z M141.352,383.418l-38.467,38.468c-5.311,5.311-5.311,13.92,0,19.23 c2.652,2.659,6.133,3.984,9.615,3.984c3.481,0,6.963-1.325,9.615-3.984l38.467-38.468c5.311-5.311,5.311-13.919,0-19.23 C155.278,378.107,146.656,378.107,141.352,383.418z M429.849,383.418c-5.311-5.311-13.92-5.311-19.23,0s-5.311,13.92,0,19.23 l38.468,38.468c2.658,2.659,6.134,3.984,9.615,3.984s6.956-1.325,9.615-3.984c5.311-5.311,5.311-13.92,0-19.23L429.849,383.418z M530.4,244.8H476c-7.507,0-13.6,6.093-13.6,13.6c0,7.507,6.093,13.6,13.6,13.6h54.4c7.507,0,13.6-6.093,13.6-13.6 C544,250.893,537.907,244.8,530.4,244.8z M420.233,137.367c3.481,0,6.956-1.326,9.615-3.985l38.468-38.468 c5.311-5.311,5.311-13.919,0-19.23c-5.311-5.311-13.92-5.311-19.23,0l-38.468,38.468c-5.311,5.311-5.311,13.919,0,19.23 C413.271,136.041,416.752,137.367,420.233,137.367z M353.601,462.4c0,7.507-6.093,13.6-13.601,13.6H231.2 c-7.507,0-13.6-6.093-13.6-13.6c0-7.242,5.678-13.11,12.818-13.519C221.952,372.354,142.8,355.307,142.8,265.2 c0-78.866,63.934-142.8,142.8-142.8c78.866,0,142.8,63.934,142.8,142.8c0,90.106-79.152,107.154-87.618,183.682 C347.922,449.29,353.601,455.158,353.601,462.4z M254.259,160.548c-2.115-5.216-8.051-7.725-13.287-5.624 c-34.755,14.083-61.104,44.186-70.482,80.525c-1.408,5.46,1.877,11.016,7.331,12.424c0.85,0.224,1.707,0.326,2.55,0.326 c4.542,0,8.684-3.053,9.874-7.65c7.766-30.11,29.594-55.053,58.385-66.715C253.851,171.721,256.367,165.777,254.259,160.548z"></path>{" "}
                                </g>{" "}
                            </g>
                        </svg>
                    </div>

                    <div className="pt-3 sm:pt-5">
                        <h2 className="text-xl font-semibold text-black dark:text-white">
                            Creative Excellence
                        </h2>

                        <p className="mt-4 text-sm/relaxed">
                            Innovation is at the heart of what we do. From
                            unique designs to cutting-edge solutions, we bring
                            creativity and expertise to every project.
                        </p>
                    </div>
                </div>
                <div></div>
            </div>
            <div className="grid xl:grid-cols-2 my-4">
                <div></div>

                <div className="flex items-start gap-4 rounded-lg bg-white p-6 shadow-[0px_14px_34px_0px_rgba(0,0,0,0.08)] ring-1 ring-white/[0.05] transition duration-300 hover:text-black/70 hover:ring-black/20 focus:outline-none focus-visible:ring-[#FF2D20] lg:pb-10 dark:bg-opacity-80 dark:bg-zinc-900 dark:ring-zinc-800 dark:hover:text-white/70 dark:hover:ring-zinc-700 dark:focus-visible:ring-[#FF2D20]">
                    <div className="flex size-12 shrink-0 p-1 items-center justify-center rounded-full bg-[#FF2D20]/10 sm:size-16">
                        <svg
                            viewBox="-6.4 -6.4 76.80 76.80"
                            xmlns="http://www.w3.org/2000/svg"
                            stroke-width="3"
                            stroke="#000000"
                            fill="none"
                        >
                            <g id="SVGRepo_bgCarrier" stroke-width="0">
                                <rect
                                    x="-6.4"
                                    y="-6.4"
                                    width="76.80"
                                    height="76.80"
                                    rx="38.4"
                                    fill="#0400ff"
                                    strokewidth="0"
                                ></rect>
                            </g>
                            <g
                                id="SVGRepo_tracerCarrier"
                                stroke-linecap="round"
                                stroke-linejoin="round"
                            ></g>
                            <g id="SVGRepo_iconCarrier">
                                <path
                                    d="M12.91,31.8V26.1a19.09,19.09,0,0,1,38.18,0v5.7"
                                    stroke-linecap="round"
                                ></path>
                                <path
                                    d="M12.06,31.8h4.7a0,0,0,0,1,0,0V45.18a0,0,0,0,1,0,0h-4.7a3,3,0,0,1-3-3V34.8A3,3,0,0,1,12.06,31.8Z"
                                    stroke-linecap="round"
                                ></path>
                                <path
                                    d="M50.24,31.8h4.7a0,0,0,0,1,0,0V45.18a0,0,0,0,1,0,0h-4.7a3,3,0,0,1-3-3V34.8A3,3,0,0,1,50.24,31.8Z"
                                    transform="translate(102.18 76.98) rotate(180)"
                                    stroke-linecap="round"
                                ></path>
                                <path
                                    d="M51.7,45.56v5a4,4,0,0,1-4,4H36.56"
                                    stroke-linecap="round"
                                ></path>
                                <rect
                                    x="28.45"
                                    y="51.92"
                                    width="8.1"
                                    height="5.07"
                                    rx="2"
                                    stroke-linecap="round"
                                ></rect>
                            </g>
                        </svg>
                    </div>

                    <div className="pt-3 sm:pt-5">
                        <h2 className="text-xl font-semibold text-black dark:text-white">
                            Unmatched Customer Support
                        </h2>

                        <p className="mt-4 text-sm/relaxed">
                            Your satisfaction is our priority. Our dedicated
                            team is always here to guide, assist, and support
                            you at every step of your journey.
                        </p>
                    </div>
                </div>
            </div>
        </section>
    );
}
